...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if view_img:
                ...
                if platform.system() != 'Linux':  # Handle non-Linux systems
    ...
    if view_img:
        if platform.system() != 'Linux':  # Cleanup on non-Linux
            cv2.destroyAllWindows()
    LOGGER.info(f'Done. ({(Profile()[0].start - Profile()[1].end) / 1e3:.3f}s)')

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--weights', type=str, default='yolo.pt', help='Model path')
    parser.add_argument('--source', type=str, default='data/images', help='Source path')
    parser.add_argument('--data', type=str, default='data/coco.yaml', help='Data configuration path')
